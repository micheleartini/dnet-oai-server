...
<xsl:template match="oai:header">
  ...
  <table class="values">
    ...
    <td class="value">
      ...
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=oaf&amp;identifier={oai:identifier}">oaf</a>
...
<xsl:template match="oai:metadata/*" priority='-100'>
  <xsl:choose>
  	<xsl:when test="name()='oaf:entity'">
  		<h3>OAF Metadata Format</h3>
  	</xsl:when>
